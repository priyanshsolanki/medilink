...
import { Heart, UserCheck, Stethoscope, Eye, EyeOff, ArrowLeft, Mail, Lock, User } from 'lucide-react';
...
export default function MediLinkLogin() {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      console.log('Login attempt:', { loginType, ...values });
      alert(`${loginType} login attempt with email: ${values.email}`);
    }
  });
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full">
          ...
          <div className="text-center mt-8">
            <button onClick={() => navigate('/')} className="text-gray-500 hover:text-gray-700 font-medium transition-colors">
              ← Back to Homepage
            </button>
          </div>
        </div>
  ...
  const subtitle = isPatient ? 'Access your health dashboard' : 'Access your practice dashboard';
        ...
        <form onSubmit={formik.handleSubmit} className="bg-white rounded-2xl shadow-xl p-8">
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-4 py-3 border rounded-xl focus:ring-2 focus:border-transparent transition-all duration-200 ${formik.touched.email && formik.errors.email ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:ring-blue-500'}`}
                  placeholder={isPatient ? "patient@example.com" : "doctor@hospital.com"}
                  required
                />
              </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={formik.handleChange}
                  onBlur={formik.handleBlur}
                  className={`w-full pl-10 pr-12 py-3 border rounded-xl focus:ring-2 focus:border-transparent transition-all duration-200 ${formik.touched.password && formik.errors.password ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:ring-blue-500'}`}
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                className={`text-sm text-${primaryColor}-600 hover:text-${primaryColor}-700 font-medium`}
              >
                Forgot password?
              </button>
            </div>
            ...
            <div className="space-y-3">
              ...
              {isPatient && (
                <button onClick={() => navigate('/patientRegister')}
                  type="button"
                  className="w-full flex items-center justify-center px-4 py-3 border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors duration-200"
                >
          ...
          <div className="mt-8 text-center">
            <p className="text-sm text-gray-600">
              {isPatient ? "Don't have a patient account?" : "New to our medical platform?"}
              <button className={`ml-1 text-${primaryColor}-600 hover:text-${primaryColor}-700 font-medium`}>
                {isPatient ? "Register here" : "Contact administrator"}
              </button>
            </p>
          </div>
        </form>
