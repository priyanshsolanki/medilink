...
const Sidebar = () => {
  ...
  const patientNavItems = [
    ...
    { name: "Chats", icon: <MessageCircle size={20} />, path: "/patient/chats" },
              ...
              {navItems.map((item) => (
                  <NavLink
                      ...
                      className={({ isActive }) => {
                        ...
                        const isChatActive =
                            (item.path === "/patient/chats" && (window.location.pathname === "/patient/chats" || window.location.pathname.startsWith("/patient/chat/"))) ||
                            (item.path === "/doctor/chats" && (window.location.pathname === "/doctor/chats" || window.location.pathname.startsWith("/doctor/chat/"))) ||
                            isActive;

                        return `flex items-center space-x-3 text-gray-700 p-3 rounded-lg w-full text-left transition ${
                            isChatActive ? "bg-blue-100 text-blue-600" : "hover:bg-blue-50"
                        }`;
                      }}
                  >
                    {item.icon}
